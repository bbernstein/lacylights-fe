...
function CueRow({
  ...
  return (
    <tr
      className={`${rowBgClass} transition-colors duration-300 border-b border-gray-200 dark:border-gray-700`}
    >
      <td
...
export default function CueListPlaybackView({
  cueListId,
  onClose,
}: CueListPlaybackViewProps) {
  ...
  const currentCue =
    currentCueIndex >= 0 && currentCueIndex < cues.length
      ...
      : null;
  const nextCue =
    currentCueIndex + 1 < cues.length ? cues[currentCueIndex + 1] : null;
            ...
            <tbody className="bg-white dark:bg-gray-800">
              {cues.map((cue: Cue, index: number) => (
                <CueRow
                  ...
                  fadeProgress={
                    index === currentCueIndex ? fadeProgress : undefined
                  }
                />
              ))}
            </tbody>

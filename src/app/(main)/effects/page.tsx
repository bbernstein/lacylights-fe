              ...
              <div key={effect.id} className="bg-white dark:bg-gray-800 shadow rounded-lg p-4 space-y-3">
                ...
                <div className="flex items-center justify-end space-x-2 pt-2 border-t border-gray-200 dark:border-gray-700">
                  <button
                    ...
                    title={activeEffectId === effect.id ? `Stop effect ${effect.name}` : `Activate effect ${effect.name}`}
                    aria-label={activeEffectId === effect.id ? `Stop effect ${effect.name}` : `Activate effect ${effect.name}`}
                  >
                    {activeEffectId === effect.id ? (
                  ...
                  <button
                    ...
                    title={`Edit effect ${effect.name}`}
                    aria-label={`Edit effect ${effect.name}`}
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <button
                    ...
                    title={`Delete effect ${effect.name}`}
                    aria-label={`Delete effect ${effect.name}`}
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center justify-end space-x-2">
                        <button
                          ...
                          aria-label={activeEffectId === effect.id ? "Stop effect" : "Activate effect"}
                        >
                          {activeEffectId === effect.id ? (
